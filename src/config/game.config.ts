...
export function generateBalancedLayout() {
  ...
  const switchConnections: Array<{id: string, source: string, target: string, x: number}> = [];
  let switchId = 1;
  ...
  for (let i = 0; i < trackNames.length - 1; i++) {
    ...
    for (let j = 0; j < numSwitches; j++) {
      const x = switchPositions[Math.floor(Math.random() * switchPositions.length)];
      const id = `switch${switchId++}`;
      switches[id] = x;
      switchConnections.push({id, source: sourceTrack, target: targetTrack, x});
    }
  }
  ...
  stopPositions.forEach((stopX, index) => {
    ...
    const switchesBeforeStop = switchConnections.filter(sw => 
      sw.source === track && sw.x < stopX - 100
    );
    
    if (switchesBeforeStop.length === 0) {
      // Add a switch before this stop
      const earlierX = stopX - 150 - Math.random() * 100;
      const targetTrack = trackNames[Math.floor(Math.random() * trackNames.length)];
      if (targetTrack !== track) {
        const id = `switch${switchId++}`;
        switches[id] = Math.round(earlierX);
        switchConnections.push({id, source: track, target: targetTrack, x: earlierX});
      }
    }
