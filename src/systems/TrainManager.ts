...
export class TrainManager {
  ...
  private spawnTrain(): void {
    ...
    const mainTracks: TrackPosition[] = ['track1', 'track2', 'track3', 'track4', 'track5'];
  ...
  private findSafeSpawnForTrack(track: TrackPosition): { speed: number } | null {
    if (!this.generatedLayout) {
      ...
      const speed = speedVariants[Math.floor(Math.random() * speedVariants.length)];
      return { speed };
    }

    // Find the rightmost (most recent) train on this track
    const trainsOnTrack = this.trains.filter(train => train.getCurrentTrack() === track);
    ...
    const upcomingStops = this.getUpcomingStopsForTrack(track, rightmostTrain.x);
    ...
    const requiredDistance = minSafeDistance + (adjustedReactionTime * pixelsPerMs);
    ...
    if (rightmostTrain.x > requiredDistance) {
      ...
      const speedVariants = GAME_CONFIG.physics.trainSpeedVariants;
      const safeSpeedOptions = speedVariants.filter(speed => speed <= maxSafeSpeed);
      
      if (safeSpeedOptions.length > 0) {
        const speed = safeSpeedOptions[Math.floor(Math.random() * safeSpeedOptions.length)];
        return { speed };
      }
    }
  ...
  private getAvailableSwitchesForTrack(track: TrackPosition): any[] {
    ...
    return this.generatedLayout.connections.filter((connection: any) => 
      connection.source === track
    );
  }
  ...
  private getUpcomingStopsForTrack(track: TrackPosition, currentX: number): any[] {
    if (!this.generatedLayout) return [];
    
    return Object.values(this.generatedLayout.stops).filter((stop: any) => 
      stop.track === track && stop.x > currentX
    );
  }
  ...
  private calculateMaxSafeSpeed(leadTrain: any): number {
    ...
    const maxAvailableSpeed = Math.max(...GAME_CONFIG.physics.trainSpeedVariants);
